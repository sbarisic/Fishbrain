param(
    ...
    [string]$OutputPath = "data/benchmarks/benchmark-base-128.jsonl"
)
...
foreach ($section in $sections) {
    ...
    for ($index = 0; $index -lt 2; $index++) {
        ...
        $band = if ($text -match '\b(?:FUCK|SHIT|DAMN|HELL)\b') { 'PROFANITY' }
            elseif ($text -match '\b(?:KILL|ATTACK|SHOOT|STAB|BOMB|FIRE)\b') { 'FICTIONAL_VIOLENCE' }
            else { 'ORDINARY' }
        $records.Add([ordered]@{

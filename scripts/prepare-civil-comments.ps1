...
used = set()
bands = [
    (frame[frame.identity_attack >= 0.5], 150),
    (frame[frame.threat >= 0.5], 150),
    (frame[frame.sexual_explicit >= 0.5], 150),
    (frame[(frame.obscene >= 0.5) | (frame.insult >= 0.5)], 300),
    (frame[(frame.obscene < 0.5) & (frame.threat < 0.5) & (frame.insult < 0.5) &
           (frame.identity_attack < 0.5) & (frame.sexual_explicit < 0.5)], 2250),
]

...
internal static partial class RuntimeTestSuite
{
    public static void RunAll()
    {
        var tests = new (string Name, Action Test)[]
        {
            ...
            ("ITEM SUBSTRING REFERENCE ISOLATION", ItemSubstringReferenceIsolation),
    ...
    static void ToolSchemaValidation()
    {
        ...
        try
        {
            _ = new GameToolRegistry([new InvalidSchemaTool()]);
        }
    ...
    static void RuntimeBoundaryValidation()
    {
        ...
        try
        {
            _ = brain.Reply(Request("hello", turnId: "BAD\u001fID"), GameToolRegistry.Empty);
        }
        catch (ArgumentException)
        {
        ...
        {
            duplicateClarificationRejected = true;
        }
    ...
    static void ItemSubstringReferenceIsolation()
    {
        var brain = TestBrain();
        var tools = DemoGameTools.CreateMerchant();
        var first = brain.Reply(Request("what do you know about europe?", turnId: "EUROPE"), tools);

        Assert(first.Diagnostics.Slots.All(slot => slot.Type != SlotType.Item),
            "an entity containing an item name must not create an item slot");
        Assert(first.State.References.Item is null,
            "an entity substring must not poison the recent item reference");

        var second = brain.Reply(Request("buy it", first.State, turnId: "BUY-IT"), tools);

        Assert(second.Diagnostics.ToolInvocation is null,
            "an unresolved pronoun must not purchase an item extracted from a substring");
    }
}

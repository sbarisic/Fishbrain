...
try
{
    ...
    if (args[0] == "compile-trajectories")
    {
        ...
        foreach (var line in File.ReadLines(args[1]))
        {
            ...
            var messages = new List<ChatMessage>(); var targets = new List<object>();
            foreach (var step in episode.Steps)
            {
                ...
                if (teacherCalls.Length > 0)
                {
                    ...
                    targets.Add(new { history = messages.ToArray(), target = protocol, authoritative = false });
                    ...
                    targets.Add(new { history = messages.ToArray(), target = JsonDefaults.Write(new { type = "text", text }), authoritative = true });
                ...
                else
                {
                    ...
                    targets.Add(new { history = messages.ToArray(), target = JsonDefaults.Write(new { type = "text", text }), authoritative = false });
                    messages.Add(new(MessageRole.Assistant, text, messages.Count));
                }
            }
    ...
    if (args[0] == "pack-corpus")
    {
        ...
        foreach (var line in File.ReadLines(Path.Combine(root, "prepared/episodes.jsonl")))
        {
            ...
            var targets = new List<(ChatMessage[] History, string Target)>();
            if (e.TryGetProperty("targets", out var supplied))
                foreach (var t in supplied.EnumerateArray()) targets.Add((JsonDefaults.Read<ChatMessage[]>(t.GetProperty("history").GetRawText()), t.GetProperty("target").GetString()!));
            else
            {
            ...
            foreach (var (history, target) in targets)
            {
                try
                {
                    ...
                    output.WriteLine(JsonDefaults.Write(new { id, family, split, pool, tokens = packed.Tokens.Concat(answer).ToArray(), promptLength = packed.Tokens.Length,
                        historyTurns = packed.Retained.Count, target })); accepted++;
                }
...
internal sealed record TeachingStep(string Player, string? Text = null, TeachingCall? Call = null, bool AllowFailure = false, TeachingCall[]? Calls = null);

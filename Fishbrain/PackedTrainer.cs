    ...
    internal sealed class Layout
    {
        public Layout(BrainConfig config, int vocabularySize, int outputSize)
        {
            ...
            for (var layer = 0; layer < config.LayerCount; layer++)
            {
                ...
                Value[layer] = offset;
                offset += size * size;
                AttentionOutput[layer] = offset;
                offset += size * size;
                MlpIn[layer] = offset;
                offset += config.MlpSize * size;
                MlpOut[layer] = offset;
                offset += size * config.MlpSize;
            }
            IntentHead = offset;
            offset += Enum.GetValues<DialogueIntent>().Length * size;
            AffectHead = offset;
            offset += Enum.GetValues<UserAffect>().Length * size;
            ExpectedHead = offset;
            offset += 2 * size;

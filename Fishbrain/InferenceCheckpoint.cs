...
public sealed partial class Brain
{
	...
	internal void ExportInference(string path, string corpusHash = "UNKNOWN")
	{
		...
		var header = new InferenceHeader
		{
			...
			ResponseCatalog = global::Fishbrain.ResponseCatalog.SurfaceCatalog.ToDictionary(
	...
	internal static bool IsInferenceCheckpoint(string path)
	{
		...
		if (stream.Length < ModelMagic.Length) return false;
		var magic = new byte[ModelMagic.Length];
	...
	private static Brain LoadInferenceCheckpoint(string path)
	{
		...
		var header = JsonSerializer.Deserialize<InferenceHeader>(headerBytes)
			?? throw new InvalidDataException("Checkpoint header is empty.");
		if (header.Format != "FISHBRAIN INFERENCE MODEL" ||
		...
		ValidateResponseCatalog(header.ResponseCatalog, global::Fishbrain.ResponseCatalog.SurfaceCatalog);
		ModelSchemas.Validate(header.LabelSchemas, header.ConfidenceCalibration);
	...
	private sealed class InferenceHeader
	{
		...
		public Dictionary<string, ModelSchemas.ConfidenceThreshold> ConfidenceCalibration { get; set; } = new(StringComparer.Ordinal);

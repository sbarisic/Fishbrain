...
internal static class ResponseCatalog
{
	...
	static ResponseCatalog()
	{
		if (Plans.Count < 200) throw new InvalidOperationException("The response catalog must contain at least 200 response plans.");
		if (Plans.Select(plan => plan.Id).Distinct(StringComparer.Ordinal).Count() != Plans.Count ||
			...
			throw new InvalidOperationException("The response catalog contains duplicate or invalid plan IDs.");
		if (Plans.Any(plan => !Enum.IsDefined(plan.Policy) ||
			...
			throw new InvalidOperationException("The response catalog contains invalid plan metadata.");
		if (Plans.SelectMany(plan => plan.Variations).Distinct(StringComparer.Ordinal).Count() < 4_400)
			throw new InvalidOperationException("The response catalog must contain at least 4,400 distinct project-owned variations.");
		if (Plans.SelectMany(plan => plan.Variations).Any(text => text is null ||
			!DialogueText.IsCanonical(text) || text.Length > 256))
			throw new InvalidOperationException("The response catalog contains invalid surface text.");
	}

...
| F080 | P0 | FIXED | Known-item slot extraction and response-plan keyword scoring still used substring matching after lexical-boundary routing had been fixed. An unrelated entity such as `EUROPE` could create a hidden `ROPE` reference and make a later `BUY IT` target the wrong item. Both paths now use one boundary-aware phrase locator, with a stateful regression. |
| F081 | P2 | FIXED | Core behavior, training, evaluation, corpus import/audit, and self-tests were concentrated in four 1,200-2,700-line files. Test executables also used top-level statements and compressed control flow. The code is split into responsibility-based partial files, both test entry points are ordinary `Program.Main` classes, and every C# file remains below 1,000 lines. |
| F082 | P1 | FIXED | Corpus compilation stored held-out inputs and cross-source external-input deduplication in mutable static sets. Consecutive or concurrent compiler use could leak state between runs. Each compile now owns an explicit `CompilationContext` passed to every external loader. |
| F083 | P0 | FIXED | Speaker-relative statements and corrections had no typed discourse frame or reducer-owned fact path. The runtime now distinguishes player and NPC participants, applies all supported fact predicates only through `DialogueStateReducer`, and keeps claims separate from authoritative state. |
| F084 | P1 | FIXED | Explanation requests could only repeat or fall into a generic response. Utterance sequences, dynamic antecedent scoring, deterministic unambiguous constraints, clarification on ties, and semantic response traces now support callbacks and repair. |
| F085 | P0 | FIXED | Corpus holdout discovery depended on the compile output directory and silently disappeared for alternate output paths. Compile and audit now resolve both mandatory benchmark files from the manifest directory and fail closed when either is missing. |
| F086 | P1 | FIXED | The Civil Comments selection had too little deterministic reserve for quote-aware normalization, cross-source deduplication, and benchmark filtering. The preparation script now retains 500 additional neutral candidates while the compiler still imports exactly 3,000 rows. |
| F087 | P1 | FIXED | The documented final curriculum did not actually train conversational realization, and its middle phase continued updating operational heads. The 200K-245K phase now isolates discourse/coreference/fact spans and ranking; the 245K-260K phase adds project-owned conversational language updates while structured heads remain frozen. |
| F088 | P0 | FIXED | Completing neural training automatically replaced `model-latest.fbm` before runtime and human gates ran. Training now retains a candidate checkpoint only; replacement requires an explicit export after every gate passes. |
| F089 | P0 | FIXED | The first 260K live session remembered a player occupation but `WHAT IS MY OCCUPATION?` fell through to NPC/world knowledge, deictic location questions treated `WE` as a place, sale-availability questions became `SELL` transactions, gift wording skipped purchase confirmation, and wares counts looked like prices. Player-fact questions now resolve before knowledge tools, activity and NPC-role callbacks use discourse memory, current-location pronouns preserve speaker perspective, availability uses `LIST_WARES`, gift requests require explicit confirmation, inventory ownership stays authoritative, and stock counts are labelled. One stateful regression runs the complete sequence against both deterministic rules and the trained artifact. |
...
| A004 | P1 | OPEN | Ranked responses are safe but often mechanical because 4,400-plus distinct visible variations are programmatically expanded. This blocks natural banter. Add a production conversational realization path plus authored dialogue packs and human preference review; keep exact facts and mutations deterministic. |
...
| A009 | P1 | OPEN | General-purpose banter and small talk are required but not implemented to an acceptable breadth. The current model is strongest on bounded game-dialogue schemas and authored response plans. Expand project-owned multi-turn social data, conversational state, realization, and held-out evaluation while keeping unsupported factual knowledge honest and tool authority unchanged. |
| A010 | P1 | OPEN | There is no dedicated general-conversation benchmark or release gate. Add held-out multi-turn sessions for topic continuity, persona consistency, follow-up relevance, humor and teasing, repetition, topic switching, unsupported-fact honesty, and human-rated appropriateness. Do not count the existing 256-turn operational benchmark as proof of this capability. |

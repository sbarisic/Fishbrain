...
| F081 | P2 | FIXED | Core behavior, training, evaluation, corpus import/audit, and self-tests were concentrated in four 1,200-2,700-line files. Test executables also used top-level statements and compressed control flow. The code is split into responsibility-based partial files, both test entry points are ordinary `Program.Main` classes, and every C# file is below 800 lines. |
